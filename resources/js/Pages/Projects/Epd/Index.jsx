...
export default function Index({ auth, projects, filters }) {
    ...
    const handleFilter = (e) => {
        e.preventDefault();
        router.get(route('projects.epd.index'), { search, status: statusFilter }, { preserveState: true });
    };
    ...
    return (
        <AuthenticatedLayout user={auth.user} header="Manajemen Proyek Kemasan">
            <Head title="EPD Relaunch" />

            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-100">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h3 className="text-lg font-bold text-gray-800">Daftar Proyek EPD Relaunch</h3>
                            <p className="text-sm text-gray-500 mt-1">Pantau seluruh pipeline pengembangan kemasan EPD dari tahap konsep hingga Go-Live.</p>
                        </div>

                        {canCreateProject && (
                            <Link
                                href={route('projects.epd.create')}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 p-6 bg-gray-50/50">
                    {projects.data.length > 0 ? projects.data.map(project => (
                        ...
                        <div className="col-span-full py-16 text-center">
                            ...
                            <h4 className="text-lg font-bold text-gray-700">Belum ada proyek kemasan EPD Relaunch</h4>
                            <p className="text-gray-500 mt-1 max-w-md mx-auto">Mulai dengan membuat pengajuan Project Brief EPD pertama Anda.</p>
                            {canCreateProject && (
                                <Link href={route('projects.epd.create')} className="inline-block mt-4 px-4 py-2 bg-indigo-600 text-white rounded-lg font-medium text-sm hover:bg-indigo-700">
                                    Buat Brief EPD Sekarang
                                </Link>
                            )}
                        </div>
                    )}
